...
class Pixel {
    ...
    constructor(R: number, C: number, color: HEX, bar: Bar) {
        this.row = R
        this.col = C
        this.color = color
        ...
        this.graphics.onclick = () => {
            ...
            this.pixelClicked(this, this.row, this.col)
        }
    }
    ...
    pixelClicked(p: Pixel, row: Number, col: Number) {
...
class HexGrid {
    ...
    constructor(rows: number, cols: number, bar: Bar) {
        this.bar = bar
        for (let row = 0; row < rows; row++) {
            this.pixels[row] = []
            for (let col = 0; col < cols; col++) {
                let newObject = new Pixel(row, col, myC.lGrey, this.bar)
